      ...
      <div class="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
        ...
        <form (ngSubmit)="onSubmit()" #f="ngForm" class="space-y-6">
          ...
          <div>
            ...
            <input id="password" name="password" [(ngModel)]="password" required type="password"
                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          </div>
          <div>
          ...
          <p class="text-sm text-center text-gray-600">
            Not a member? <a routerLink="/register" class="font-medium text-indigo-600 hover:text-indigo-500">Register now</a>
          </p>
        </form>
      </div>
    </div>
...
export class LoginComponent {
  ...
  onSubmit() {
    ...
    this.auth.login(this.email, this.password).subscribe({
      ...
      error: err => { this.error = err?.error?.message || 'Login gagal'; this.loading = false; }
    });
  }
}
